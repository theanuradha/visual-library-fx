...
public final class ConnectionWidgetLayout implements Layout {
	...
	private final boolean useStacking = true; // if false, then it is backward
												// compatible; if true, then
												// child widgets with the same
												// placement are stacked
												// vertically
	...
	public void setConstraint(Widget childWidget, ConnectionWidgetLayoutAlignment alignment,
	...
	private void layoutChildrenAt(Point linePoint, ConnectionWidgetLayoutAlignment alignment,
			ConnectionWidget connectionWidget, ArrayList<Widget> children) {
		if (alignment == null)
	...
	private void layoutStackedChildrenAt(Point linePoint, ConnectionWidgetLayoutAlignment adjustedAlignment,
			ConnectionWidget connectionWidget, ArrayList<Widget> children) {
		...
		Point referencePoint = getReferencePointForAdjustedAlignment(adjustedAlignment,
				new Rectangle(areaWidth, areaHeight));
	...
	private void layoutSingleChildAt(Point linePoint, ConnectionWidgetLayoutAlignment alignment,
			ConnectionWidget connectionWidget, Widget childWidget) {
		Rectangle preferredBounds = childWidget.getPreferredBounds();
		...
		childWidget.resolveBounds(new Point(linePoint.x - referencePoint.x, linePoint.y - referencePoint.y),
				preferredBounds);
	}

	private ConnectionWidgetLayoutAlignment getAdjustedHorizontalAlignment(
			ConnectionWidgetLayoutAlignment adjustedAlignment) {
		switch (adjustedAlignment) {
		case TOP_LEFT:
		case CENTER_LEFT:
		case BOTTOM_LEFT:
	...
	private Point getReferencePointForAdjustedAlignment(ConnectionWidgetLayoutAlignment adjustedAlignment,
			Rectangle rectangle) {
		switch (adjustedAlignment) {
		case BOTTOM_CENTER:
	...
	private ConnectionWidgetLayoutAlignment getAdjustedAlignment(ConnectionWidgetLayoutAlignment alignment,
