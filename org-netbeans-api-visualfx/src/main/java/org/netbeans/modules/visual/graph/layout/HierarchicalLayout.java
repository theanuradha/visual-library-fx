...
public class HierarchicalLayout<N, E> extends GraphLayout<N, E> {
	...
	public HierarchicalLayout(GraphScene<N, E> scene, boolean animate, boolean inverted, int xOffset, int layerOffset) {
		...
		// scene is not used yet. It will be used when the container agnostic
		// feature
	...
	private abstract class AlgorithmPart {
		...
		public void start() {
			...
			if (TRACE) {
				System.out.println(
						"Timing for " + this.getClass().getName() + " is " + (System.currentTimeMillis() - start));
				printStatistics();
			}
			...
			for (int i = 1; i < layerCount; i++) {
				...
				for (LayoutNode n : layers[i]) {
					...
					if (n.preds.size() > 0) {
						...
						// if(n.vertex == null) n.crossingNumber +=
						// layers[i].size();
					}
				}
	...
	private class WriteResult extends AlgorithmPart {
		...
		protected void run() {
				...
				for (LayoutEdge e : n.succs) {
					if (e.link != null) {
						...
						while (cur.vertex == null && cur.succs.size() != 0) {
							// if(points.size() > 1 && points.get(points.size()
							// -1).x == cur.x + cur.width/2 &&
							// points.get(points.size() - 2).x == cur.x +
							// cur.width/2) {
			...
			for (E l : linkPositions.keySet()) {
				...
				// Kris - this is a hack to reverse the order of the control
				// points
				// that were created by the algorithm. This is used when the
				// graph
				// is inverted.
