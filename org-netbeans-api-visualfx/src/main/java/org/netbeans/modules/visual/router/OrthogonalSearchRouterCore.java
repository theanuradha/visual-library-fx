...
final class OrthogonalSearchRouterCore {
	...
	private static final int MAXIMAL_DEPTH = 5; // 6 is too slow for 10+ nodes
												// and 10+ links
	private static final int CORNER_LENGTH = 200;
	private static final boolean UPDATE_COLLISION_LISTS = true; // set true for
																// faster
																// computation
																// but not
																// optimal
	...
	static final boolean IGNORE_LINKS_WITH_ACTUAL_PORTS = false; // cause links
																	// to try to
																	// merge as
																	// a bus,
																	// multilinks
	...
	public OrthogonalSearchRouter.Solution route() {
		...
		// fixing a routing bug. If the x or y coordinates are on top of each
		// other,
		...
		OrthogonalSearchRouterRegion region = new OrthogonalSearchRouterRegion(sourceBoundaryPoint.x,
				sourceBoundaryPoint.y, 0, 0, sourceDirection, 0);

		search(region);

		// something went wrong in the search
		if (bestControlPoints == null)
			return null;

		Solution solution = new OrthogonalSearchRouter.Solution(bestControlPointsPrice,
				Arrays.asList(bestControlPoints));
	...
	private void search(OrthogonalSearchRouterRegion region) {
		...
		region = new OrthogonalSearchRouterRegion(inter.x, inter.y, inter.width, inter.height, region.getDirection(),
				region.getDepth());
		...
		if (region.containsInsideEdges(targetBoundaryPoint)) {
			// TODO - not true - point could lay on right or bottom edge ! -
	...
	private void constructControlPoints(int depth) {
		...
		if (desiredDirection != regions[depth].getDirection()) {
			...
			depth++;
			regions[depth] = new OrthogonalSearchRouterRegion(region.x, region.y, region.width, region.height,
					desiredDirection, depth);
		}
		...
		for (int a = 0; a < depth; a++) {
			...
			if (region.isHorizontal()) {
				...
				if (a > 0) {
					...
					if (y1 <= OrthogonalSearchRouterRegion.MIN_INT_REGION
							&& y2 < OrthogonalSearchRouterRegion.MAX_INT_REGION)
						yy = y2 - OrthogonalSearchRouter.SPACING_EDGE;
					else if (y1 > OrthogonalSearchRouterRegion.MIN_INT_REGION
							&& y2 >= OrthogonalSearchRouterRegion.MAX_INT_REGION)
				...
				if (a > 0) {
					...
					if (x1 <= OrthogonalSearchRouterRegion.MIN_INT_REGION
							&& x2 < OrthogonalSearchRouterRegion.MAX_INT_REGION)
						xx = x2 - OrthogonalSearchRouter.SPACING_EDGE;
					else if (x1 > OrthogonalSearchRouterRegion.MIN_INT_REGION
							&& x2 >= OrthogonalSearchRouterRegion.MAX_INT_REGION)
		...
		// if (newY >= regionY.y && newY < regionY.y + regionY.height && newY >=
		// regionY1.y && newY < regionY1.y + regionY1.height)
		...
		// if (newX >= regionX.x && newX < regionX.x + regionX.width && newX >=
		// regionX1.x && newX < regionX1.x + regionX1.width)
	...
	private int calculatePrice(int controlPointsLength, Point[] controlPoints) {
		...
		// add the lengths of the legs (cheap distance formula) and a corner
		// value
		...
		// paths that have leg lengths out of proportion (non-square'ish), have
		// a
		// higher price.
