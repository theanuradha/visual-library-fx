...
public class BirdViewWindow extends JWindow implements MouseMotionListener {
	...
	public void setWindowSize(Dimension size) {
		...
		if (isShowing()) {
			...
			setLocation(location.x + (previousSize.width - size.width) / 2,
					location.y + (previousSize.height - size.height) / 2);
			validate();
		}
	}
	...
	private void updateForViewPoint(Point viewPoint) {
		...
		setBounds(viewOrigin.x + viewPoint.x - size.width / 2, viewOrigin.y + viewPoint.y - size.height / 2, size.width,
				size.height);
		setVisible(true);
		birdView.repaint();
	}
	...
	private void updateForBirdViewPoint(Point birdViewPoint) {
		...
		if (view.isShowing() && isShowing()) {
			...
			Dimension size = getSize();
			updateForViewPoint(new Point(birdViewPoint.x + birdViewOrigin.x - viewOrigin.x,
					birdViewPoint.y + birdViewOrigin.y - viewOrigin.y));
		} else
			updateForViewPoint(null);
	}
