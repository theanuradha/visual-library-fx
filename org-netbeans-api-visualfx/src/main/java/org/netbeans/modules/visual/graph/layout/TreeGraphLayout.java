...
public final class TreeGraphLayout<N, E> extends GraphLayout<N, E> {
	...
	/**
	 * Indicates whether the gap between two nodes of the same level should be
	 * minimized.
	 */
	 ...
	 * @param horizontalGap
	 *            the horizontal gap between cells
	 * @param vertical
	 *            if true, then layout organizes the graph vertically; if false,
	 *            then horizontally
	 */
	 ...
	 *            then horizontally
	 * @param minimizeGap
	 *            if true, then minimize the gap between cells; if false do the
	 *            normal tree layout.
	 * @param alignment
	 *            alignment the alignment of the nodes in their level. Choose
	 *            wheter {@code TreeGraphLayout.Alignment.TOP},
	 *            {@code TreeGraphLayout.Alignment.CENTER} or
	 *            {@code TreeGraphLayout.Alignment.BOTTOM}
	 * @since 2.25
	 */
	public TreeGraphLayout(int originX, int originY, int verticalGap, int horizontalGap, boolean vertical,
			boolean minimizeGap, TreeGraphLayoutAlignment alignment) {
	...
	private class Node {
		...
		private class LeftRight {
			 ...
			 *
			 * @param left
			 *            the most left coordinate of this {@link LeftRight}.
			 * @param right
			 *            the most right coordinate of this {@link LeftRight}.
			 * @since 2.25
			 */
			 ...
			 * 
			 * @param left
			 *            the most left coordinate to set.
			 * @since 2.25
			 */
			 ...
			 * 
			 * @param left
			 *            the most right coordinate to set.
			 * @since 2.25
			 */
		...
		/**
		 * This method calculates the maximal space for every level and returns
		 * the result as a {@link Map} containing the level (which is equal to
		 * the depth of the tree) and the height/width of the biggest node in
		 * this level.
		 * 
		 * @param graph
		 * @return the result as a {@link Map} containing the level (which is
		 *         equal to the depth of the tree) and the height/width of the
		 *         biggest node in this level.
		 * @since 2.25
		 */
		...
		/**
		 * Method to determine the maximal space (horizontaly or verticaly) of
		 * each level in the tree.
		 * 
		 * @param map
		 *            the {@link Map} to which the maximal space for each level
		 *            should be stored.
		 * @param lvl
		 *            the level to analyse.
		 * @return the result of the maximal space calculation for the given
		 *         level and the sublevels.
		 * @since 2.25
		 */
		private Map<Integer, Integer> calculateMaxSpace(UniversalGraph<N, E> graph, Map<Integer, Integer> map,
				int lvl) {
		...
		/**
		 * Method which is doing the layout based on the given x - position, y -
		 * position, map of levels and their maximal used space and the actual
		 * level.
		 * 
		 * @param x
		 *            the x position for doing the layout.
		 * @param y
		 *            the y position for doing the layout.
		 * @param lvl
		 *            the auctual level for doing the layout.
		 * @return the envelope of the subtrees for which the layout is already
		 *         done.
		 * @since 2.25
		 */
		private List<LeftRight> layout(int x, int y, Map<Integer, Integer> map, int lvl) {
			...
			// if this node is a leaf (has no childs) ==> place the node at x,y
			// and x and x + width to the envelope.
			if (children.size() == 0) {
			...
			for (int i = 0; i < children.size(); i++) {
				if (i == 0) {
					...
					List<LeftRight> secound = children.get(i).layout(x, (int) (y + map.get(lvl - 1) + verticalGap), map,
							lvl);
					...
					// Variable used for calculation of the distance to move the
					// right subtree.
					int moveDist = Integer.MIN_VALUE;

					// Try to minimize the gap and move subtrees as close as
					...
					// store the overall envelope of leftright and secound in
					// leftright
					for (int j = rightlength - 1; j >= 0; j--) {
						// leftright wasn't as long as secound, so these
						// elements have to be added to the envelope
						if (j < secound.size() - leftright.size()) {
							leftright.add(0, secound.get(j));
						} else if ((j - diff) >= 0 && j >= 0) {
							// if the left envelope of secound is smaller than
							// the left envelope of leftright, might never
							// happen.
							if (leftright.get(j - diff).left > secound.get(j).left) {
								leftright.get(j - diff).setLeft(secound.get(j).left);
							}
							// if the right envelope of secound is greater than
							// the right envelope of leftright, might happen
							// every time when a subtree was added by a move to
							// the right.
			...
			if (minimizeGap) {
				if (vertical) {
					point = new Point(
							((leftright.get(leftright.size() - 1).right + leftright.get(leftright.size() - 1).left) / 2)
									- (relativeBounds.width / 2),
							yAlignment - relativeBounds.y);
					leftright.add(new LeftRight(point.x, point.x + relativeBounds.width));
				} else {
					point = new Point(
							((leftright.get(leftright.size() - 1).right + leftright.get(leftright.size() - 1).left) / 2)
									- (relativeBounds.height / 2),
							yAlignment - relativeBounds.x);
				...
				if (vertical) {
					point = new Point(((leftMin + rightMax) / 2) - (relativeBounds.width / 2),
							yAlignment - relativeBounds.y);
					leftright.add(new LeftRight(point.x, point.x + relativeBounds.width));
				} else {
					point = new Point(((leftMin + rightMax) / 2) - (relativeBounds.height / 2),
							yAlignment - relativeBounds.x);
					leftright.add(new LeftRight(point.x, point.x + relativeBounds.height));
				}
			}
		 ...
		 * 
		 * @param dx
		 *            the distance to move the children horizontally.
		 * @since 2.25
		 */
		...
		/**
		 * Doing the alignment whether on TOP, in the CENTER or at the BOTTOM of
		 * the level.
		 * 
		 * @param y
		 *            the top y coordinate of the level.
		 * @param map
		 *            the map of levels and their maximal spaces.
		 * @param lvl
		 *            the level for which the alignment should be done.
		 ...
		 * 
		 * @param moveDistance
		 *            the distance to move the subtree (x-axis).
		 * @since 2.25
		 */
