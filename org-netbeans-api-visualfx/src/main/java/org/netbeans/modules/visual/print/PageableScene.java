...
/**
 * This is an implementation class for the public ScenePrinter class. The
 * ScenePrinter instantiates a PageableScene which is then sent to the
 * PrinterJob.
 * 
 * @author krichard
 */
public class PageableScene implements Pageable, Printable {
	...
	/**
	 * Creates an instance of a PageableScene used with a PrinterJob to print
	 * the Scene.
	 * 
	 * @param scene
	 *            the Scene to be printed
	 * @param format
	 *            the format to which the printed scene will adhere (legal,
	 *            letter, custom). If the format parameter is null, a default
	 *            letter (8.5x11) is created and used.
	 * @param scaleType
	 *            type of scaling to be done before printing. The options
	 *            are<br/>
	 *            <ul>
	 *            <li>NO_SCALING</li>
	 ...
	 *            ScaleStrategy.SCALE_PERCENT. Otherwise it is ignored.
	 */
	public PageableScene(Scene scene, PageFormat format, ScaleStrategy scaleType, double scaleX, double scaleY,
		...
		if (visibleOnly || region != null) {
			...
			// by the scene. Since setScaledSize calculates the new size by
			// getting
			// the scene dimensions and multiplying it by the scale, then we
			// send
			// 1.0 since the scene size has already been adjusted by the
			// zoomFactor.
			setScaledSize(1.0, 1.0);
		} else if (scaleType.equals(ScaleStrategy.NO_SCALING)) {
			// restore the correct number of pages
			// Is it true that the scene size does not decrease if the
			// zoomfactor
			// is less than one??
	 ...
	 * 
	 * @param graphics
	 *            Graphics object for the PrinterJob.
	 * @param pageFormat
	 *            PageFormat to adhere to while printing.
	 * @param pageIndex
	 *            The current page number.
	 * @return NO_SUCH_PAGE if the current page number is not valid and
	 *         PAGE_EXISTS otherwise.
	...
	/**
	 * Adjusts the scaling factors in both the horizontal and vertical
	 * directions to garuntee that the Scene prints onto a single page.
	 * 
	 * @param useSymmetricScaling
	 *            if true, the horizontal and vertical scaling factors will be
	 *            the same whereby preserving the current aspect ration. The
	 *            smallest of the two (horizontal and vertical) scaling factors
	 *            is used for both.
	 */
	private void scaleToFit(boolean useSymmetricScaling) {
		PageFormat format = getPageFormat();

		if (scene.getView().getWidth() * scene.getView().getHeight() == 0) {
			return;
		}
		double scaleX = format.getImageableWidth() / scene.getView().getWidth();
		double scaleY = format.getImageableHeight() / scene.getView().getHeight();
		...
		if (scaleX < 1 || scaleY < 1) {
			...
			setSize((float) (scene.getView().getWidth() * scaleX), (float) (scene.getView().getHeight() * scaleY));
			setScaledSize(scaleX, scaleY);
	...
	/**
	 * Set the print size to fit a page in the verticle direction. The
	...
	/**
	 * Set the absolute scaling values for both the horizontal and vertical
	 * dimensions.
	 * 
	 * @param scaleX
	...
	/**
	 * Returns the number of pages that the printing will use to print at the
	 * set zoom level.
	 * 
	 * @return the number of pages that the printing will use to print at the
	 *         set zoom level.
	 */
	 ...
	 * 
	 * @param pageFormat
	 *            the PageFormat that will be adhered to during printing.
	 */
	 ...
	 * 
	 * @param pageIndex
	 *            the page number for which the PageFormat is sought.
	 * @return the PageFormat that will be adhered to during printing the given
	 *         page.
