...
/**
 * Override at least isReplacementWidget and setConnectionAnchor methods.
 * reconnectingFinished is always called before setConnectionAnchor.
 *
 * @author David Kaspar
 */
public final class ReconnectAction extends WidgetAction.LockedAdapter {
	...
	public State mousePressed(Widget widget, WidgetMouseEvent event) {
		...
		if (event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 1) {
			if (widget instanceof ConnectionWidget) {
				...
				} else if (controlPoints != null && index == controlPoints.size() - 1
						&& provider.isTargetReconnectable(conn)) {
	...
	private void cancel(Point point) {
		...
		if (point != null)
			if (Math.abs(floatPoint.x - point.x) >= ReconnectAction.MIN_DIFFERENCE
					|| Math.abs(floatPoint.y - point.y) >= ReconnectAction.MIN_DIFFERENCE)
	...
	public State keyPressed(Widget widget, WidgetKeyEvent event) {
		if (isLocked() && event.getKeyCode() == KeyCode.ESCAPE) {
			cancel(null);
