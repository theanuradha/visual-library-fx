 ...
 * @author David Kaspar
 */
public final class InplaceEditorAction<C extends JComponent> extends WidgetAction.LockedAdapter
	...
	private boolean openEditor(Widget widget, InplaceEditorProvider.EditorInvocationType invocationType) {
		...
		if (true)
			throw new RuntimeException("TODO: Inplace editor");
		Scene scene = widget.getScene();
		Node component = scene.getView();
		if (component == null) {
			return false;
		}
	...
	private void updateRectangleToFitToView(Rectangle rectangle) {
		SceneNode component = widget.getScene().getView();
		if (rectangle.x + rectangle.width > component.getWidth()) {
			rectangle.x = (int) component.getWidth() - rectangle.width;
		}
		if (rectangle.y + rectangle.height > component.getHeight()) {
			rectangle.y = (int) component.getHeight() - rectangle.height;
		}
		if (rectangle.x < 0) {
			rectangle.x = 0;
		}
	...
	public void notifyEditorComponentBoundsChanged() {
		EnumSet<InplaceEditorProvider.ExpansionDirection> directions = provider.getExpansionDirections(this, widget,
				editor);
		if (directions == null) {
