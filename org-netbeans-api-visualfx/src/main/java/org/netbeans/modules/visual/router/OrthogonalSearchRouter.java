...
public final class OrthogonalSearchRouter implements Router {
	...
	public java.util.List<Point> routeConnection(ConnectionWidget widget) {
		/*
		 * How this works: 1) get the source and target anchors 2) determine
		 * collisions - needed for the actual routing 3) get the centers of the
		 * source and target anchor 4) check to see if the anchor will allow the
		 * route to connect to any side. This really means that the router can
		 * test every orthogonal direction out of the center of the anchor. If
		 * not allowed then get the attach point from the anchor and use it to
		 * determine the route. 5) Iterate through the different allowed
		 * orthogonal directions from the target and source to find the actual
		 * "best" route. This route or Solution with contain a list of points
		 * for the route. 6) Send the list of best points to the source anchor
		 * and allow it to alter the points. The assumption is that the anchor
		 * will only move the end point to the appropriate intersection point.
		 * 7) Repeat (6) for the target. 8) Now that the "best" points have been
		 * moved they may not represent an orthogonal path any longer. Calculate
		 * the directions coming from/to the anchors in preparation for
		 * re-routing. 9) Using the new endpoints(6&7) and the directions(8),
		 * re-route and get the updated solution.
		 * 
		 */
		...
		if (sourceAnchor.getRelatedWidget() != null
			...
			Rectangle rec = sourceAnchor.getRelatedWidget()
					.convertLocalToScene(sourceAnchor.getRelatedWidget().getBounds());
			rec.grow(SPACING_NODE, SPACING_NODE);
			verticalCollisions.add(rec);
			horizontalCollisions.add(rec);
		}

		// the default is to point to the center of any anchor. If an extended
		// anchor
		// overwrites this class to return a different point than the center,
		// then
		// drawing problem will exist. Meaning that the connection may draw over
		// the
		...
		// set the default test direction to be ANY in order to test all
		// directions
		// to find the best path.
		EnumSet<Direction> sourceDirections = Anchor.DIRECTION_ANY;
		EnumSet<Direction> targetDirections = Anchor.DIRECTION_ANY;

		// if the anchor does not allow arbitrary connection points, then ask
		// the
		...
		for (Anchor.Direction sourceDirection : sourceDirections) { // up to
																	// four
																	// choices
																	// (TOP,
																	// BOTTOM,
			...
			for (Anchor.Direction targetDirection : targetDirections) {// up to
																		// four
																		// choices
																		// (TOP,
																		// BOTTOM,
				...
				Solution solution = new OrthogonalSearchRouterCore(widget.getScene(), verticalCollisions,
						horizontalCollisions, sourcePoint, sourceDirection, targetPoint, targetDirection).route();
		...
		if (sourceAnchor.allowsArbitraryConnectionPlacement()) {
			// the initial route went to the center of the anchor. Now that we
			// have the
			// "best" route, ask the anchor to where to place its end point.
			// These calls
			// only adjust the end points within the anchor. Example would be
			// with the
			// rectangular anchor. The connection is meant to terminate on the
			// boundary
			// and not at the center. Since the "best" route ends at the center,
			// it must
			// be moved to the boundary.
			bestPoints = sourceAnchor.compute(bestListOfPoints);

			// best points now contain points that may no longer be orthogonal
			// due to the
			// adjusting of the endpoints. In order to re-align the points,
			// first determine
			// the direction that the points attach to the anchor and then
			// re-route from the
			// best attach points.
			// Note: this methodology will fail miserably if the anchor moves
			// the points
			// around around they have been initially set. Meaning that this
			// method assumes
			// that the only sections of the connection not orthogonal are
			// possibly the
			// the first and last.
			...
			if (deltaX == 0) { // if deltaX is zero then we have a vertical
								// connection

				sourceDirection = originalSourceCenterPoint.y < firstPointOutsideAnchor.y ? Direction.BOTTOM
						: Direction.TOP;
			} else {
				sourceDirection = originalSourceCenterPoint.x < firstPointOutsideAnchor.x ? Direction.RIGHT
						: Direction.LEFT;
			}
		}
		...
		if (targetAnchor.allowsArbitraryConnectionPlacement()) {
			...
			if (deltaX == 0) {
				targetDirection = originalTargetCenterPoint.y < firstPointOutsideAnchor.y ? Direction.BOTTOM
						: Direction.TOP;
			} else {
				targetDirection = originalTargetCenterPoint.x < firstPointOutsideAnchor.x ? Direction.RIGHT
						: Direction.LEFT;
			}
		...
		// re-route
		if (sourceAnchor.allowsArbitraryConnectionPlacement() || targetAnchor.allowsArbitraryConnectionPlacement()) {
			bestSolution = new OrthogonalSearchRouterCore(widget.getScene(), verticalCollisions, horizontalCollisions,
					bestPoints.get(0), sourceDirection, bestPoints.get(bestPoints.size() - 1), targetDirection).route();
		}
