...
/**
 * This class represents a connection between two location. The locations are
 * resolved by Anchors. The path of the connection is specified by control
 * points which are resolved by Routers (DirectRouter is used by default).
 * <p>
 * The connection is rendered using the foreground color and a specified stroke.
 * It also renders control points, end points (first and last control points)
 * and anchors. The shape of points are defined by assigned AnchorShape and
 * PointShape.
 * <p>
 * For speed optimalization, the connection widget are usually placed in the a
 * separate layer widget that is rendered after the main layer with widgets
 * (that used anchors depends on) is rendered.
 * <p>
 * Line color is defined by foregroundColor property. Note: If you are changing
 * a state of the ConnectionWidget (e.g. using it as a representation of an
 * object in ObjectScene, GraphScene or GraphPinScene classes, then the
 * ConnectionWidget.notifyStateChanged method is automatically called. The
 * built-in implementation of this method overrides foregroundColor based on a
 * new state of the widget (the particular color is resolved by the LookFeel of
 * the scene).
 *
 * @author David Kaspar
 */
// TODO - control points can be modified by accessing: getControlPoints ().get
// (0).x or y
public class ConnectionWidget extends Widget {
	...
	/**
	 * This enum represents a policy which is used for re-routing control points
	...
	public enum RoutingPolicy {
		...
		/**
		 * All control points are rerouted when it is necessary. This is the
		 * default policy.
		 * 
		 * @since 2.9
		 */
		 ...
		 * End points are updated to locations resolved by source and target
		 * anchors. Note: This is used when an user customizes/adds/removes
		 * control points and the change has to be kept until it is reset by the
		 * user (setting policy to ALWAYS_ROUTE.
		 * 
		 * @since 2.9
		 */
		...
		/**
		 * Temporarily disables routing until any end point changes its
		 * location. Locations are the first and the last point of control
		 * points. When an end point location is changed, then the policy is
		 * automatically changed to <code>ALWAYS_ROUTE</code>. Note: This is
		 * used by GraphLayouts which routes the path (control points) by
		 * themselves and would like to keep the path until user moves with
		 * source or target widget/anchor.
		 * 
		 * @since 2.9
		 */
		...
		/**
		 * Disable routing completely, so control points are kept at their
		 * previous location. Note: This is not often used unless you have to
		 * freeze a ConnectionWidget
		 * 
		 * @since 2.9
		 */
		DISABLE_ROUTING
	 ...
	 * 
	 * @param lineColor
	 *            the line color; if null, then the line color will be resolved
	 *            from LookFeel of the scene.
	 */
	 ...
	 * 
	 * @param controlPointsCursor
	 *            the control points cursor
	 * @since 2.3
	 */
	 ...
	 * 
	 * @param controlPointCutDistance
	 *            if positive number, then the path is cut to render smooth
	 *            corners; otherwise the path is rendered using control points
	 *            only
	 * @since 2.5
	 */
	 ...
	 *            if true, then controlPoints argyment is taken as a list of
	 *            scene locations; if false, then controlPoints argument is
	 *            taken as a list of local locations
	 */
	 ...
	 * 
	 * @param childWidget
	 *            the child widget for which the constraint is set
	 * @param alignment
	 *            the alignment specified relatively to the origin point
	 * @param placementInPercentage
	 *            the placement on a path in percentage of the path length
	 */
	public void setConstraint(Widget childWidget, LayoutFactory.ConnectionWidgetLayoutAlignment alignment,
	 ...
	 * @param alignment
	 *            the alignment specified relatively to the origin point
	 * @param placementAtDistance
	 *            the placement on a path in pixels as a distance from the
	 *            source anchor
	 */
	public void setConstraint(Widget childWidget, LayoutFactory.ConnectionWidgetLayoutAlignment alignment,
	...
	public final void calculateRouting() {
		if (routingRequired) {
			...
			case UPDATE_END_POINTS_ONLY: {
				Point sourcePoint = sourceAnchor != null ? sourceAnchor.compute(sourceEntry).getAnchorSceneLocation()
						: null;
				Point targetPoint = targetAnchor != null ? targetAnchor.compute(targetEntry).getAnchorSceneLocation()
						: null;
			...
			case DISABLE_ROUTING_UNTIL_END_POINT_IS_MOVED: {
				...
				if (sourcePoint == null ? firstPoint == null : sourcePoint.equals(firstPoint)) {
					Point targetPoint = targetAnchor != null
							? targetAnchor.compute(targetEntry).getAnchorSceneLocation() : null;
	...
	protected Rectangle calculateClientArea() {
		...
		Rectangle rect = null;
		for (Point point : controlPoints) {
			Rectangle addRect = new Rectangle(point.x - controlPointShapeRadius, point.y - controlPointShapeRadius,
					controlPointShapeRadius2, controlPointShapeRadius2);
			if (rect == null)
				rect = addRect;
			else
				rect.add(addRect);
	...
	/**
	 * Returns a cursor for a specified local location in the widget. If
	 * paintControlPoints is true and controlPointsCursor is non-null and local
	 * location is over a control point, then it return controlPointsCursor.
	...
	/**
	 * Paints the connection widget (the path, the anchor shapes, the control
	 * points, the end points).
	 */
	protected void paintWidget() {
		...
		// checking to see if we should draw line through the AnchorShape. If
		// the
		...
		double firstControlPointRotation = firstControlPoint != null
				&& (sourceAnchorShape.isLineOriented() || isSourceCutDistance) ? getSourceAnchorShapeRotation() : 0.0;
		double lastControlPointRotation = lastControlPoint != null
				&& (targetAnchorShape.isLineOriented() || isTargetCutDistance) ? getTargetAnchorShapeRotation() : 0.0;
		...
		if ((isSourceCutDistance || isTargetCutDistance) && controlPoints.size() >= 2) {
			...
			points.set(0, new Point(
					firstControlPoint.x
							+ (int) (sourceAnchorShape.getCutDistance() * Math.cos(firstControlPointRotation)),
					firstControlPoint.y
							+ (int) (sourceAnchorShape.getCutDistance() * Math.sin(firstControlPointRotation))));
			points.set(controlPoints.size() - 1,
					new Point(
							lastControlPoint.x
									+ (int) (targetAnchorShape.getCutDistance() * Math.cos(lastControlPointRotation)),
							lastControlPoint.y
									+ (int) (targetAnchorShape.getCutDistance() * Math.sin(lastControlPointRotation))));
		} else {
			...
			for (int a = 0; a < points.size() - 1; a++) {
				...
				if (a > 0) {
					...
					if (!Double.isNaN(direction)) {
						path = addToPath(path, p1.x + (int) (cll * Math.cos(direction)),
				...
				if (a < points.size() - 2) {
					...
					double direction = Math.atan2(p2.y - p1.y, p2.x - p1.x);
					if (!Double.isNaN(direction)) {
						path = addToPath(path, p2.x - (int) (cll * Math.cos(direction)),
