...
/**
 * Represents an event for ContiguousSelectEvent passed to
 * ContiguousSelectProvider. Contains information about selection-type,
 * previously and currently choosen objects spots.
 *
 * @author David Kaspar
 * @since 2.17
 */
public final class ContiguousSelectEvent {
	...
	private ContiguousSelectEvent(Widget previouslyChoosenWidget, Point previouslyChoosenLocalLocation,
			Widget choosenWidget, Point choosenLocalLocation, SelectionType selectionType) {
	 ...
	 * @param previousLocalLocation
	 *            the local location of the previously choosen widget
	 * @param choosenWidget
	 *            the choosen widget
	 * @param choosenLocalLocation
	 *            the local location of the currently choosen widget
	 * @param selectionType
	 *            the selection type invoked by an user
	 * @return the contiguous select event
	 */
	public static ContiguousSelectEvent create(Widget previousWidget, Point previousLocalLocation, Widget choosenWidget,
			Point choosenLocalLocation, SelectionType selectionType) {
		assert selectionType != null;
		return new ContiguousSelectEvent(previousWidget, previousLocalLocation, choosenWidget, choosenLocalLocation,
				selectionType);
	}
	...
	public enum SelectionType {
		...
		/**
		 * Represents a normal selection that replace the previous selection.
		 * All objects that are between previously and current choosen spots
		 * defines the current selection. Usually invokes with Shift
		 * key-modifier.
		 */
		REPLACE_CONTIGUOUS,

		/**
		 * Represents an additive selection where the new selection should be
		 * added to the current selection. Usually invokes with Ctrl
		 * key-modifier.
		 */
		...
		/**
		 * Represents an additive selection where the new selection should be
		 * added to the current selection. All objects that are between
		 * previously and current choosen spots defines the current selection.
