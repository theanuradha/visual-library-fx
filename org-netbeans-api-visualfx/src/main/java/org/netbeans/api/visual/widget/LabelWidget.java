...
/**
 * A widget representing a text. The widget is not opaque and is checking
 * clipping for by default.
 * <p>
 * It allows to set 4 types of horizontal and vertical alignments (by default
 * LEFT as horizontal and BASELINE as vertical).
 * <p>
 * Swing Font-hinting feature may cause the labels are not rendered completely.
 * Using <code>setUseGlyphVector</code> you may force the label to be converted
 * to a glyph vector which scales correctly for any zoom factor.
 *
 * @author David Kaspar
 */
public class LabelWidget extends Widget {
	...
	public enum Orientation {
		NORMAL, ROTATE_90// , ROTATE_180, ROTATE_270, MIRROR, MIRROR_ROTATE_90,
							// MIRROR_ROTATE_180, MIRROR_ROTATE_270
	}
	...
	/**
	 * Returns whether the label widget is using glyph vector for rendering
	 * text.
	 * 
	 ...
	 * Note that if you are not using glyph vector then the text may be clipped
	 * when a scene has zoom factor different from 1.0.
	 * 
	 * @param useGlyphVector
	 *            if true, then a glyph vector is used for rendering text
	 * @since 2.7
	 */
	...
	private void assureGlyphVector() {
		...
		cacheFont = font;
		cacheLabel = label;
		cacheGlyphVector = font.createGlyphVector(new FontRenderContext(new AffineTransform(),
				fontRenderContext.isAntiAliased(), fontRenderContext.usesFractionalMetrics()), cacheLabel);
	}
	...
	protected Rectangle calculateClientArea() {
		...
		if (useGlyphVector) {
			...
			rectangle.grow(1, 1); // WORKAROUND - even text antialiasing is
									// included into the boundary
		} else {
