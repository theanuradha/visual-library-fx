...
/**
 * This a scroll widget similar to JScrollPane. A scrolled widget is could be
 * manipulated with getView and setView methods. The scroll-bars are
 * automatically visible when the scrolled widget is bigger than the widget.
 *
 * @author David Kaspar
 */
// TODO - does not cooperate with action-tools - actions from null-tool should
// be working all the time
// TODO - invalid calculation/checking showVertical and showHorizontal when
// view.preferredLocation has negative values
public class ScrollWidget extends Widget {
	...
	private Widget createDownArrow() {
		return new ButtonWidget(getScene(),
				Utilities.loadImage("org/netbeans/modules/visual/resources/arrow-down.png")); // NOI18N
	}

	private Widget createLeftArrow() {
		return new ButtonWidget(getScene(),
				Utilities.loadImage("org/netbeans/modules/visual/resources/arrow-left.png")); // NOI18N
	}

	private Widget createRightArrow() {
		return new ButtonWidget(getScene(),
				Utilities.loadImage("org/netbeans/modules/visual/resources/arrow-right.png")); // NOI18N
	}
	...
	private Dimension calculateSize() {
		if (isPreferredBoundsSet()) {
			...
			return new Dimension(preferredBounds.width - insets.left - insets.right,
	...
	private final class ScrollLayout implements Layout {
		...
		public void layout(Widget widget) {
			...
			Rectangle viewportBounds = view != null ? new Rectangle(view.getLocation(), calculateSize())
					: new Rectangle();
			...
			if (showVertical) {
				...
				downArrow.resolveBounds(new Point(x2, y2 - BAR_VERTICAL_SIZE),
						new Rectangle(BAR_HORIZONTAL_SIZE, BAR_VERTICAL_SIZE));
				verticalSlider.resolveBounds(new Point(x2, y1 + BAR_VERTICAL_SIZE), new Rectangle(BAR_HORIZONTAL_SIZE,
						viewportBounds.height - BAR_VERTICAL_SIZE - BAR_VERTICAL_SIZE));
			} else {
			...
			if (showHorizontal) {
				...
				rightArrow.resolveBounds(new Point(x2 - BAR_HORIZONTAL_SIZE, y2),
						new Rectangle(BAR_HORIZONTAL_SIZE, BAR_VERTICAL_SIZE));
				horizontalSlider.resolveBounds(new Point(x1 + BAR_HORIZONTAL_SIZE, y2), new Rectangle(
						viewportBounds.width - BAR_HORIZONTAL_SIZE - BAR_HORIZONTAL_SIZE, BAR_VERTICAL_SIZE));
			} else {
			...
			verticalSlider.setValues(viewBounds.y + viewportBounds.y,
					viewBounds.y + viewportBounds.y + viewBounds.height, 0, viewportBounds.height);
			horizontalSlider.setValues(viewBounds.x + viewportBounds.x,
					viewBounds.x + viewportBounds.x + viewBounds.width, 0, viewportBounds.width);
		}
		...
		private boolean checkHorizontal(Rectangle viewBounds, Rectangle viewportBounds) {
			return (viewBounds.x < viewportBounds.x
					|| viewBounds.x + viewBounds.width > viewportBounds.x + viewportBounds.width)
					&& viewportBounds.width > 3 * BAR_HORIZONTAL_SIZE;
		}

		private boolean checkVertical(Rectangle viewBounds, Rectangle viewportBounds) {
			return (viewBounds.y < viewportBounds.y
					|| viewBounds.y + viewBounds.height > viewportBounds.y + viewportBounds.height)
					&& viewportBounds.height > 3 * BAR_VERTICAL_SIZE;
		}
	...
	private static class SliderWidget extends Widget {
		...
		public float getPixelIncrement() {
			return (float) (vertical ? getClientArea().height : getClientArea().width)
			...
			if (minimumValue < maximumValue && startValue < endValue) {
				...
				if (start + 4 < end) {
					if (vertical) {
						...
						BORDER.paint(gr,
								new Rectangle(clientArea.x, clientArea.y + start, clientArea.width, end - start));
					} else {
						gr.fillRect(clientArea.x + start + 2, clientArea.y + 2, end - start - 4, clientArea.height - 4);
						BORDER.paint(gr,
								new Rectangle(clientArea.x + start, clientArea.y, end - start, clientArea.height));
					}
				} else {
					if (vertical)
