 ...
 * <p>
 * In comparison with the GraphScene class, in this class the graph consists of
 * nodes, edges and pins. Each pin is assigned to a node (the assigned cannot be
 * change at any time. Each edge could be attach to a single source and target
 * pin.
 * <p>
 * The class is abstract and manages only data model and mapping with widgets.
 * The graphics (widgets) has to be supplied by a developer by overriding the
 * attachNodeWidget, attachPinWidget, attachEdgeWidget, attachEdgeSourceAnchor
 * and attachEdgeTargetAnchor abstract methods.
 * <p>
 * This class is using generics and allows you to specify type representation
 * for nodes and edges in the graph model. Example:
 * 
 * <pre>
 * class MyGraph extends GraphScene&lt;MyNode, MyEdge, MyPin&gt; { ... }
 * </pre>
 * 
 * Since the type of nodes, edges and pins could be the same, all node, edge and
 * pin instances have to be unique within the whole scene.
 *
 * @author David Kaspar
 */
...
public abstract class GraphPinScene<N, E, P> extends ObjectScene {
	 ...
	 * 
	 * @param node
	 *            the node to be added; the node must not be null, must not be
	 *            already in the model, must be unique in the model (means:
	 *            there is no other node, edge or pin in the model has is equal
	 *            to this node) and must not be a Widget
	 ...
	 * 
	 * @param node
	 *            the node to be removed; the node must not be null and must be
	 ...
	 * 
	 * @param edge
	 *            the edge to be added; the edge must not be null, must not be
	 *            already in the model, must be unique in the model (means:
	 *            there is no other node, edge or pin in the model has is equal
	 *            to this edge) and must not be a Widget
	 ...
	 * 
	 * @param edge
	 *            the edge to be removed; the edge must not be null and must be
	 *            already in the model
	 */
	 ...
	 * 
	 * @param node
	 *            the node where the pin is assigned to
	 * @param pin
	 *            the pin to be added; the pin must not be null, must not be
	 *            already in the model, must be unique in the model (means:
	 *            there is no other node, edge or pin in the model has is equal
	 *            to this pin) and must not be a Widget
	 * @return the widget that is created by attachPinWidget; null if the pin is
	 *         non-visual
	 */
	 ...
	 * 
	 * @param pin
	 *            the pin to be removed; the pin must not be null and must be
	 *            already in the model
	 */
	 ...
	 * 
	 * @param edge
	 *            the edge which source is going to be changed
	 * @param sourcePin
	 *            the source pin; if null, then the edge source will be detached
	 */
	 ...
	 * 
	 * @param edge
	 *            the edge which target is going to be changed
	 * @param targetPin
	 *            the target pin; if null, then the edge target will be detached
	 */
	 ...
	 * 
	 * @param pin
	 *            the pin which edges connections are searched for
	 * @param allowOutputEdges
	 *            if true, the output edges are included in the collection; if
	 *            false, the output edges are not included
	 * @param allowInputEdges
	 *            if true, the input edges are included in the collection; if
	 *            false, the input edges are not included
	...
	/**
	 * Called by the addNode method to notify that a node is added into the
	 * graph model.
	 * 
	 * @param node
	 *            the added node
	 * @param widget
	 *            the widget created by the attachNodeWidget method as a visual
	...
	/**
	 * Called by the addEdge method to notify that an edge is added into the
	 * graph model.
	 * 
	 * @param edge
	 *            the added node
	 * @param widget
	 *            the widget created by the attachEdgeWidget method as a visual
	...
	/**
	 * Called by the addPin method to notify that a pin is added into the graph
	 * model.
	 * 
	 * @param node
	 *            the node where the pin is assigned to
	 ...
	 *            the added pin
	 * @param widget
	 *            the widget created by the attachPinWidget method as a visual
	...
	/**
	 * Called by the removeEdge method to notify that an edge is removed from
	 * the graph model. The default implementation removes the edge widget from
	 * its parent widget.
	 * 
	 * @param edge
	 *            the removed edge
	...
	/**
	 * Called by the removePin method to notify that a pin is removed from the
	 * graph model. The default implementation removes the pin widget from its
	 * parent widget.
	 * 
	 * @param pin
	 *            the removed pin
	 ...
	 *            the node where the pin is assigned to
	 * @param pin
	 *            the pin that is going to be added
	 ...
	 * 
	 * @param edge
	 *            the edge which source is changed in graph model
	 * @param oldSourcePin
	 *            the old source pin
	 * @param sourcePin
	 *            the new source pin
	 ...
	 * 
	 * @param edge
	 *            the edge which target is changed in graph model
	 * @param oldTargetPin
	 *            the old target pin
	 * @param targetPin
	 *            the new target pin
	...
	/**
	 * This class is a particular GraphPinScene where nodes, edges and pins are
	 * represented with String class.
	 */
