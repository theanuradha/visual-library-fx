...
/**
 * This class represents a graph-orinted layout. The layout is invoked using
 * layoutGraph methods. The layoutNodes method could be called for resolving
 * locations of a sub-set of all nodes.
 * <p>
 * Internally the invoked method creates an UniversalGraph from the scene in the
 * arguments. Then it calls the particular (performGraphLayout or
 * performNodesLayout) methods with the UniversalGraph as a parameter. These
 * protected methods are implemented by the GraphLayout based class and performs
 * particular layout using an UniversalGraph.
 *
 * @author David Kaspar
 */
public abstract class GraphLayout<N, E> {
	...
	public final void removeGraphLayoutListener(GraphLayoutListener<N, E> listener) {
		synchronized (listeners) {
			// .add replaced with .remove for fix to bug
			// http://netbeans.org/bugzilla/show_bug.cgi?id=197502
			// listeners.add (listener);
	...
	/**
	 * Invokes resolving of locations for a collection of nodes in a
	 * GraphPinScene.
	 * 
	 * @param graphPinScene
	 *            the graph pin scene
	 ...
	 *            the universal graph
	 * @param node
	 *            the node with resolved location
	...
	/**
	 * Implements and performs particular graph-oriented algorithm of a
	 * UniversalGraph. Call <code>GraphLayout.setResolvedNodeLocation</code>
	...
	/**
	 * Implements and performs particular location resolution of a collection of
	 * nodes in a UniversalGraph. Call
	 * <code>GraphLayout.setResolvedNodeLocation</code> method for setting the
	 * resolved node location.
	 * 
	 * @param graph
	 *            the universal graph on which the nodes should be resolved
	 * @param nodes
