...
/**
 * The widget which applies a convolve filter to a graphics rendered by the
 * children.
 * <p>
 * Children are painted to an offscreen buffer which is later painted with a
 * convolve filter applied to it.
 * <p>
 * Because of the offscreen buffer, be careful about the size of the widget. The
 * buffer stays allocated even after the painting and it is also expanding only
 * (when required). You can clear the buffer using clearCache method.
 *
 * @author David Kaspar
 */
public class ConvolveWidget extends Widget {
	...
	/**
	 * Paints the children into the offscreen buffer and then the buffer is
	 * rendered regularly using the convolve operation.
	 */
