...
/**
 * A scene is a tree of small building blocks called widgets and represented by
 * this class.
 * <p>
 * Each widget has a origin location specified relatively to the location its
 * parent widget and placement is specified be its boundary.
 * <p>
 * The widget is also responsible for rendering the region. The widget is an
 * abstract implementation and does not have render anything except borders and
 * background. There are various built-in widget each for a specific
 * visualization. The widget also holds general properties like foreground,
 * opacity, ... that could be reused by the high-level widgets.
 * <p>
 * The widget has a layout assigned. The layout takes care about resolving the
 * placement of children widgets. For that it can use various properties like
 * preferredLocation, preferredBounds, ... When the widget is resolved (placed)
 * than the read only location and bounds properties contains resolved location
 * and boundary of a widget.
 * <p>
 * Each widget has a chain of actions. Actions defined defines a behaviour of
 * the widget. E.g. MoveAction makes the widget moveable. Also there is possible
 * to create/assign other chains that will be activated based on the active tool
 * of a scene.
 * <p>
 * The widget have its state specified by ObjectState class. When the widget
 * state is change, notifyStateChanged is called to notify about it. The state
 * is automatically updated by high-level scenes and actions. Yherefore you can
 * define your own look and feel directly in the that method.
 *
...
public class Widget implements Lookup.Provider {
	...
	private static final HashMap<String, WidgetAction.Chain> EMPTY_HASH_MAP = new HashMap<String, WidgetAction.Chain>(
			0);
	...
	/**
	 * The resource table is created lazily. A resource is only created when a
	 * property is set by using using resources.
	 */
	...
	/**
	 * Returns a Graphics2D instance with is assigned to the scene. Usually used
	 * in the calculatedClientArea and paintWidget method.
	 * 
	 * @return the Graphics2D instance; null if the scene view is not created or
	 *         visible yet
	 */
	 ...
	 *            the child widget to be added
	 * @param constraint
	 *            the constraint assigned to the child widget
	 */
	 ...
	 * 
	 * @param index
	 *            the index (the child is added before the one that is not the
	 *            index place)
	 * @param child
	 *            the child widget
	 */
	 ...
	 * 
	 * @param widgets
	 *            the list of children widgets to be removed
	 */
	...
	/**
	 * This method is called to notify that the view is shown. Note: You must
	 * not modify a tree of widgets from within this method. It means: do not
	 * call addChild, removeChild and similar methods.
	 */
	protected void notifyAdded() {
	}

	/**
	 * This method is called to notify that the view is hidden. Note: You must
	 * not modify a tree of widgets from within this method. It means: do not
	 * call addChild, removeChild and similar methods.
	 */
	...
	public final Collection<Dependency> getDependencies() {
		return dependencies != null ? Collections.unmodifiableCollection(dependencies)
	...
	public final void setBackgroundFromResource(String property) {
		...
		if (table != null) {
			if ((oldPropertyName != null) && (oldPropertyName.length() > 0)) {
				// Maybe the property name has changed. Therefore, remove the
				// old
				// listener.
	 ...
	 *            the preferred location; if null, then the preferred location
	 *            is unset
	 */
	...
	/**
	 * Called to calculate the client area required by the widget without the
	 * children widgets.
	 * 
	 ...
	 * @param localLocation
	 *            the local location
	 * @return the mouse cursor; default implementation return value of cursor
	 *         property.
	 * @since 2.3
	 */
	 ...
	 * returned. See <strong>Layout</strong> section in documentation.
	 * 
	 ...
	 * 
	 * @param location
	 *            the resolved location; if null then [0,0] point is used
	 *            instead
	 * @param bounds
	 *            the resolved bounds; if null then the preferred bounds are
	 *            used instead
	 */
	 ...
	 */
	// NOTE - has to be called before a change is set into the widget when the
	// change immediatelly affects calculation of the local/scene
	// location/boundary (means any property used in convertLocalToScene)
	// because repaint/revalidate needs to calculate old scene boundaries
	 ...
	 * 
	 * @param repaintOnly
	 *            if true, then the widget is scheduled for repainting only; if
	 *            false, then widget is scheduled for revalidation (the
	 *            Scene.validate method has to be called after all changes to
	 *            invoke validation)
	 */
	// NOTE - has to be called before a change is set into the widget when the
	// change affects the local/scene location/boundary because
	// repaint/revalidate needs to calculate old scene boundaries
	...
	/**
	 * Returns whether whole area of the widget has to be repainted after the
	 * validation of the widget. Used be LayerWidget for performance
	 * optiomalization.
	 * 
...
//						bounds.height - insets.top - insets.bottom);
	...
	/**
	 * Called to paint the widget background itself only using the Graphics2D
	 * instance acquired from Scene.getGraphics method.
	 */
	protected void paintBackground() {
		...
		if (border.isOpaque())
			...
			gr.fillRect(bounds.x + insets.left, bounds.y + insets.top, bounds.width - insets.left - insets.right,
					bounds.height - insets.top - insets.bottom);
	}

	/**
	 * Called to paint the widget border itself only using the Graphics2D
	 * instance acquired from Scene.getGraphics method.
	 * 
	 * @since 2.1
	 */
	...
	/**
	 * Called to paint the widget itself only using the Graphics2D instance
	 * acquired from Scene.getGraphics method. Do not call methods which modify
	 * state of the widget (all methods which calls revalidate like setFont(),
	 * setVisible(), setLayout(),...). It can cause another repaint and result
	 * in infinite paint cycle.
	 */
	protected void paintWidget() {
	}

	/**
	 * Called to paint the children widgets only using the Graphics2D instance
	 * acquired from Scene.getGraphics method.
	 */
	...
	/**
	 * The dependency listener which is used for notifying dependent widgets,
	 * anchor, ... that the widget (or one of its parent widget) location or
	 * bounds are going to or were changed.
	 */
	public interface Dependency {

		/**
		 * Called when the widget (or one of its parent widget) location or
		 * bounds are going to or were changed.
		 */
