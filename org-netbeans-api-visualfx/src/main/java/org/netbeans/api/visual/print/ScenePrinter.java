...
public final class ScenePrinter {
	 ...
	 *            The Scene to be printed.
	 * @param scaleStrategy
	 *            Value representing the scaling strategy to use for the
	 *            printing.
	 */
	 ...
	 *            The Scene to be printed.
	 * @param scaleX
	 *            scaling percentage in the horizontal dimension.
	 * @param scaleY
	 *            scaling percentage in the vertical dimension.
	 */
	public static void print(Scene scene, double scaleX, double scaleY) {
		print(scene, null, ScaleStrategy.SCALE_PERCENT, scaleX, scaleY, false, false, null, null);
	}
	 ...
	 * scale type parameter take precedent over the horizontal and vertical
	 * scaling percentages. Therefore, if the scale type was "SCALE_TO_FIT_Y",
	 * then the values of horizontal and vertical scaling percentages would not
	 * be used.
	 * 
	 * @param scene
	 *            The Scene to be printed.
	 ...
	 * @param scaleStrategy
	 *            value representing the how to scale the printing.
	 * @param scaleX
	 *            Directly set the horizontal scale percentage. This parameter
	 *            is only used when the scale strategy is
	 *            ScaleStrategy.SCALE_PERCENT. Otherwise it is ignored.
	 * @param scaleY
	 *            Directly set the vertical scale percentage. This parameter is
	 *            only used when the scale strategy is
	 *            ScaleStrategy.SCALE_PERCENT. Otherwise it is ignored.
	 * @param selectedOnly
	 *            Print only the objects from the Scene that have been selected.
	 *            Note that in this case the Scene must be an instnace of an
	 *            ObjectScene since this is required to determine the selected
	 *            objects.
	 * @param visibleOnly
	 *            Print only the object in the visible window.
	 * @param region
	 *            The rectangle representing the are of the Scene to be printed.
	 * @param hiddenLayerWidgets
	 *            Layer that are not to be printed. Might be used to hide the
	 *            background while printing.
	 */
	public static void print(Scene scene, PageFormat format, ScaleStrategy scaleStrategy, double scaleX, double scaleY,
			boolean selectedOnly, boolean visibleOnly, Rectangle region, List<LayerWidget> hiddenLayers) {
		...
		PageableScene ps = new PageableScene(scene, format, scaleStrategy, scaleX, scaleY, selectedOnly, visibleOnly,
				region);
		...
		if (printJob.printDialog()) {
			...
			try {
				// this can not go here because it hides all the widgets
				// everytime.
				...
				// if selectedOnly is true then we need to hide all the
				// non-selected
				// widgets. Note that if the widgets were already hidden due to
				// hiding
				...
				// Similar to the selectedOnly, if visibleOnly is true we have
				// to
				// hide the widgets not in the visible window. The reason for
				// this
				// is because setting the size alone does not hide the widgets.
				// Any
				// page that is printed will include all its contained widgets
				// if
				...
				if (visibleOnly || region != null) {
					...
					// if the region is null, then the user is not trying to
					// print a
					// region. The other way into this condition is if we need
					// to
					// to print the visibleArea. So we set the region to be the
					// visible area.
					if (region == null)
						throw new RuntimeException("TODO: visibleOnly print");// region
																				// =
	...
	// this is necessary since we can print any generic scene. Meaning that we
	// can't
	...
	private static List<Widget> getAllNodeWidgets(Widget widget, List<Widget> widgetList) {
		...
		if (widget instanceof LayerWidget && widget.getChildren().size() > 0) // widget
																				// has
																				// children
		{
	...
	public enum ScaleStrategy {
		...
		/**
		 * Scale the printed scene to exactly the same scale level as the
		 * supplied scene.
		 */
		SCALE_CURRENT_ZOOM,
		/**
		 * Scale the printed scene by a supplied percent. This will scale the X
		 * and Y dimensions equally.
		 */
		SCALE_PERCENT,
		/**
		...
		SCALE_TO_FIT,
		/**
		 * Determine the scale percentage necessay to fit on the horizontal
		 * page. The vertical is scaled equally but no garuntees are made on the
		 * page fit.
		 */
		SCALE_TO_FIT_X,
		/**
		 * Determine the scale percentage necessay to fit on the vertical page.
		 * The horizontal is scaled equally but no garuntees are made on the
		 * page fit.
		 */
		SCALE_TO_FIT_Y,
		/**
		 * Print the Scene exactly as it would be if not scaled at all. Note
		 * that this is different from SCALE_CURRENT_ZOOM where the page is
		 * printed as shown visually, including the zoom.
		 */
		NO_SCALING
	}
}
