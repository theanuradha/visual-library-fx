 ...
 * The action is notified about all Swing events related to the widget where the
 * action is assigned.
 * <p>
 * Events: mouse, mouse-motion, mouse-wheel, drag'n'drop, focus, keyboard.
 * <p>
 * Each event handler has to return a state of the event processing (represented
 * by WidgetAction.State).
 * <p>
 * Each Swing event is processed by all actions of all widgets in a scene. The
 * processing can be stopped by consuming the event. When processing event for a
 * widget, then all children widget (from the last) of widget are asked for
 * processing first. Then finally the event is processed by all actions (from
 * the first to the last) assigned to the widget.
 *
 * @author David Kaspar
 */
public interface WidgetAction {
	...
	public static abstract class State {
		...
		/**
		 * The state that means: The event is not processed by the action and
		 * has to be processed by other actions too.
		 */
		...
		/**
		 * The state that means: The event is processed by the action and the
		 * processing has to stopped immediately (no other action should
		 * processed it).
		 */
		...
		/**
		 * The state that means: The event is processed by the action and only
		 * actions that are in the same chain can process the event too.
		 */
		...
		/**
		 * Creates a state that means: The event is processed and the processing
		 * has to stopped immediately (no other action should processed it).
		 * Next event will be processed by the lockedAction on lockedAction
		 * first. Only if the lockedAction reject the event, then the event will
		 * be processed regularly. It is used for locking the event processing
		 * for a long-term operation like moving, resizing, rectangular
		 * selection, ... (usually they are using mouse motion events).
		 */
		 ...
		 * processing by the chain where the actions is added.
		 * 
		...
		/**
		 * Returns whether (and by which widget) the next event has to be
	 ...
	 * 
	 * @param widget
	 *            the widget where the action is assigned
	 * @param event
	 *            the mouse event
	...
	/**
	 * An adapter of the widget action. All methods return locked or rejected
	 * event state based on a result of isLocked method. This is often used for
	 * long-term actions like MoveAction.
	 */
	public static abstract class LockedAdapter implements WidgetAction {

		/**
		 * Called by event handlers to resolve whether they should return locked
		 * or rejected event state.
		 * 
		 * @return if true, then locked event state is used; if false, then
		 *         rejected event state is used.
		 */
	...
	public static final class Chain implements WidgetAction {
		 ...
		 * 
		 * @param index
		 *            the index; the action will be added before the action at
		 *            the index position
		 * @param action
		 *            the action to be added
		 */
	...
	public static interface WidgetLocationEvent extends WidgetEvent {
		...
		/**
		 * Sets a new location. This is called by a event provider only. Do not
		 * call this method unless you know what it does exactly.
		 * 
		 * @param point
		 *            the new location
		 */
		...
		/**
		 * Translates the stored location. Do not call this method unless you
	...
	public static final class WidgetMouseEvent implements WidgetLocationEvent {
		...
		private long id;
		private javafx.scene.input.MouseEvent event;
		private int x, y;
		 ...
		 *            the Swing event
		 */
		public WidgetMouseEvent(long id, javafx.scene.input.MouseEvent event) {
			this.id = id;
			this.event = event;
			x = (int) event.getX();
			y = (int) event.getY();
	...
	public static final class WidgetKeyEvent implements WidgetEvent {
		...
		private long id;
		private javafx.scene.input.KeyEvent event;
		 ...
		 *            the Swing event
		 */
		public WidgetKeyEvent(long id, javafx.scene.input.KeyEvent event) {
			this.id = id;
			this.event = event;
		}
	...
	public static final class WidgetDropTargetDragEvent implements WidgetLocationEvent {
		 ...
		 *            the Swing event
		 */
		public WidgetDropTargetDragEvent(long id, DragEvent event) {
			this.id = id;
			this.event = event;
		 ...
		 */
		public Dragboard getTransferable() {
		 ...
		 * @see DropTargetDragEvent
		 */
		public void acceptDrag(TransferMode... dragOperation) {
			event.acceptTransferModes(dragOperation);
		}
	...
	public static final class WidgetDropTargetDropEvent implements WidgetLocationEvent {
		 ...
		 *            the Swing event
		 */
		public WidgetDropTargetDropEvent(long id, DragEvent event) {
			this.id = id;
			this.event = event;
		 ...
		 */
		public TransferMode getDropAction() {
			return event.getAcceptedTransferMode();
		}
		 ...
		 * @see DropTargetDropEvent
		 */
		public void acceptDrop(TransferMode... dragOperation) {
			event.acceptTransferModes(dragOperation);
		}
	...
	public static final class WidgetDropTargetEvent implements WidgetEvent {
		 ...
		 *            the Swing event
		 */
		public WidgetDropTargetEvent(long id, DragEvent event) {
			this.id = id;
			this.event = event;
		}
