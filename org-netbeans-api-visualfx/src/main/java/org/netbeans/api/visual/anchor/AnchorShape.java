...
/**
 * Represents an anchor shape which is rendered at the source and the target
 * point of a connection widget where the shape is used. The same instance of a
 * shape could be shared by multiple connection widgets.
 * 
 * @author David Kaspar
 */
public interface AnchorShape {
	...
	/**
	 * Returns a distance by which a line at particular source or target point
	 * should be cut (not rendered). This is used for hollow-triangle shapes, to
	 * not paint the connection-line within the triangle.
	 * 
	 * @return the cut distance in pixels; if positive, then the line is cut by
	 *         specified number of pixels, the line could be cut by radius
	 *         pixels only; if 0.0, then the line is not cut; if negative, then
	 *         the line is extended by specified number of pixels, the line
	 *         could be extended by radius pixels only
	 */
	 ...
	 *            the graphics
	 * @param source
	 *            true, if the shape is used for a source point; false if the
	 *            shape is used for a target point.
	 */
