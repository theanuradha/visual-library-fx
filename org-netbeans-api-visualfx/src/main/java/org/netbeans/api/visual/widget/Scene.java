 ...
 * <p>
 * After all changes in a scene is done, the validate method have to be called
 * for validating changed and calculating new locations and boundaries of all
 * modified widgets.
 * <p>
 * The scene allows to create a view JComponent which can be used anywhere in
 * Swing based application. Only one view can be created using the createView
 * method. The scene allows to create multiple satellite views using the
 * createSatelliteView method. The satellite view is just showing the scene and
 * allows quick navigator and panning using a mouse.
 * <p>
 * The scene contains additional scene-specific properties like lookFeel,
 * activeTool, defaultFont, animator.
 * <p>
...
public class Scene extends Widget {
	...
	private SceneNode component = null;
	...
	private Rectangle maximumBounds = new Rectangle(Integer.MIN_VALUE / 2, Integer.MIN_VALUE / 2, Integer.MAX_VALUE,
			Integer.MAX_VALUE);

	private final ArrayList<SceneListener> sceneListeners = new ArrayList<SceneListener>(); // TODO
																							// -
																							// use
	 ...
	 */
	public SceneNode createView() {
		assert component == null;
		component = new SceneNode(this);
	 ...
	 * 
	 * @param graphics
	 *            the graphics instance used for validation
	 * @since 2.7
	 */
	...
	// TODO - maybe it could improve the perfomance, if bounds != null then do
	// nothing
	// WARNING - you have to asure that there will be no component/widget that
	// will change its location/bounds between this and validate method calls
	...
	// TODO - requires optimalization while changing preferred size and calling
	// revalidate/repaint
	private void layoutScene() {
		...
		if (extendSceneOnly && rect != null && preBounds != null)
			rect.add(new Rectangle(preBounds.x + preLocation.x, preBounds.y + preLocation.y, preBounds.width,
					preBounds.height));
		...
		preferredSize = new Dimension((int) (preferredSize.width * zoomFactor),
				(int) (preferredSize.height * zoomFactor));
		Rectangle bounds = getBounds();
		if (component != null) {
			final javafx.scene.canvas.Canvas canvas = component.getCanvas();
			if (!(preferredSize.width == canvas.getWidth()) && preferredSize.height == canvas.getHeight()) {
				canvas.setWidth(preferredSize.width);
				canvas.setHeight(preferredSize.height);
				component.draw();
				bounds = getBounds();
				// repaintSatellite ();
			}

			Dimension componentSize = new Dimension((int) canvas.getWidth(), (int) canvas.getHeight());
	...
	private void resolveRepaints() {
		...
		// NOTE - maybe improves performance when component.repaint will be
		// called for all widgets/rectangles separately
		if (repaintRegion != null) {
	 ...
	 * 
	 * @param extendSceneOnly
	 *            if true, the scene is going to be extended only
	 */
	 ...
	 * 
	 * @param focusedWidget
	 *            the focused widget; if null, then the scene itself is taken as
	 *            the focused widget
	 */
	 ...
	 * 
	 * @param activeTool
	 *            the active tool; if null, then the active tool is unset and
	 *            only default action chain of widgets will be used
	 */
	...
	public final Point convertSceneToView(Point sceneLocation) {
		...
		return new Point((int) (zoomFactor * (location.x + sceneLocation.x)),
				(int) (zoomFactor * (location.y + sceneLocation.y)));
	}
	...
	public final Rectangle convertSceneToView(Rectangle sceneRectangle) {
		Point location = getLocation();
		return GeomUtil.roundRectangle(new Rectangle2D.Double((double) (sceneRectangle.x + location.x) * zoomFactor,
				(double) (sceneRectangle.y + location.y) * zoomFactor, (double) sceneRectangle.width * zoomFactor,
	...
	public Point convertViewToScene(Point viewLocation) {
		return new Point((int) ((double) viewLocation.x / zoomFactor) - getLocation().x,
				(int) ((double) viewLocation.y / zoomFactor) - getLocation().y);
	}
	...
	/**
	 * The scene listener which is notified about repainting, validating
	 * progress.
	 */
