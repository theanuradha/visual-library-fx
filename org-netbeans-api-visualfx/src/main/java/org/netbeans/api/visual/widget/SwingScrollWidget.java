...
/**
 * This is a scroll widget similar to JScrollPane. In comparison with the
 * ScrollWidget class, this class is using JScrollBar for vertical and
	...
	private final class ScrollLayout implements Layout {
		...
		public void layout(Widget widget) {
			...
			if (showVertical) {
				...
				verticalWidget.resolveBounds(new Point(x2, y1),
						new Rectangle(SwingScrollWidget.BAR_HORIZONTAL_SIZE, viewportBounds.height));
			} else {
				if (verticalWidget.getParentWidget() != null)
			...
			if (showHorizontal) {
				...
				horizontalWidget.resolveBounds(new Point(x1, y2),
						new Rectangle(viewportBounds.width, SwingScrollWidget.BAR_VERTICAL_SIZE));
			} else {
				if (horizontalWidget.getParentWidget() != null)
			...
			verticalScroll.setValues(-viewportBounds.y, viewportBounds.height, viewBounds.y,
			...
			horizontalScroll.setValues(-viewportBounds.x, viewportBounds.width, viewBounds.x,
		...
		private boolean checkHorizontal(Rectangle viewBounds, Rectangle viewportBounds) {
			return (viewBounds.x < viewportBounds.x
					...
					&& viewportBounds.width > 3 * SwingScrollWidget.BAR_HORIZONTAL_SIZE;
		}
		...
		private boolean checkVertical(Rectangle viewBounds, Rectangle viewportBounds) {
			return (viewBounds.y < viewportBounds.y
					...
					&& viewportBounds.height > 3 * SwingScrollWidget.BAR_VERTICAL_SIZE;
		}
