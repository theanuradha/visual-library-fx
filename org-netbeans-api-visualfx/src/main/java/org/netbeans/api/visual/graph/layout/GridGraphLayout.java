...
/**
 * This class represents a graph-oriented layout which organizes nodes into a
 * grid.
 * <p>
 * You can call setChecker method to enable checker style. If the parameter is
 * true, that the layout uses only half of nodes on a grid like a single color
 * on a chess board.
 * <p>
...
public final class GridGraphLayout<N, E> extends GraphLayout<N, E> {
	 ...
	 * 
	 * @param checker
	 *            if true, then checker style is enabled and nodes are organized
	 *            like a single color on a chess board.
	...
	protected void performGraphLayout(UniversalGraph<N, E> graph) {
		...
		for (N node : allNodes) {
			...
			if (bounds == null)
				continue;
			Point grid = node2grid.get(node);
			setResolvedNodeLocation(graph, node,
					new Point(xAxis[grid.x - gridBounds.x] - bounds.x, yAxis[grid.y - gridBounds.y]));
		}
	}
