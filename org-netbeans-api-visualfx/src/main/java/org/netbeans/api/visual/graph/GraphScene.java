 ...
 * nodes and edges. Each edge could be attach to a single source and target
 * node.
 * <p>
 ...
 * attachNodeWidget, attachEdgeWidget, attachEdgeSourceAnchor and
 * attachEdgeTargetAnchor abstract methods.
 * <p>
 ...
 * </pre>
 * 
 * Since the type of nodes and edges could be the same, all node and edge
 * instances has to be unique within the whole scene.
 * <p>
 * Node and Edge should not be a Widget. It should work properly but in that
 * case the ObjectScene class is loosing its purpose because there is no need to
 * manage a mapping of an instance to the same instance.
 *
 * @author David Kaspar
 */
public abstract class GraphScene<N, E> extends ObjectScene {
	 ...
	 *            there is no other node or edge in the model has is equal to
	 *            this node) and must not be a Widget
	 ...
	 *            already in the model, must be unique in the model (means:
	 *            there is no other node or edge in the model has is equal to
	 *            this edge) and must not be a Widget
	 ...
	 * @param sourceNode
	 *            the source node; if null, then the edge source will be
	 *            detached
	 */
	 ...
	 * @param targetNode
	 *            the target node; if null, then the edge target will be
	 *            detached
	 */
	 ...
	 * 
	 * @param node
	 *            the node which edges connections are searched for
	...
	/**
	 * Returns a collection of edges that are between a specified source and
	 * target nodes.
	 * 
	 * @param sourceNode
	 *            the source node
	 * @param targetNode
	 *            the target node
	 * @return the collection of edges with the specified source and target
	 *         nodes
	 */
	 ...
	 * source node to the edge in the visual representation.
	 * <p>
	 ...
	 * target node to the edge in the visual representation.
	 * <p>
	 * Usually it is implemented as:
	 * 
	 * <pre>
	...
	/**
	 * This class is a particular GraphScene where nodes and edges are
