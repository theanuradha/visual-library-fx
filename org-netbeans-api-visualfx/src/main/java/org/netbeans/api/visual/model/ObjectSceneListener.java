...
/**
 * This interface represents a listener that is notified about changes in a
 * object scene which is passed as the first parameter of every listener
 * methods.
 * <p>
 * Implementers of this interface should try and make the implemented methods as
 * brief as possible. These methods are called in the same thread that is
 * handing the corresponding action, and taking too much time in these methods
 * will slow the UI update down.
 *
...
public interface ObjectSceneListener {
	 ...
	 * called when an object-widget mapping is registered in an ObjectScene
	 * only. At the moment of the call, the object is still not reqistered in
	 * the Graph*Scene classes yet. Therefore do not use the methods of
	 * Graph*Scene.
	 * 
	 * @param event
	 * @param addedObject
	 ...
	 * called when an object-widget mapping is unregistered in an ObjectScene
	 * and Graph*Scene classes. At the moment of the call, a widget (visual
	 * representation of the object) is still in the scene. Therefore do not
	 * rely on a tree of widgets of the scene.
	 * 
	 * @param event
	...
	/**
	 * Called to notify that the object state of an object is changed. This
	 * method is always called before any other ObjectSceneListener method is
	 * called.
	 * 
	 * @param event
	 *            the object scene event
	 ...
	 */
	void objectStateChanged(ObjectSceneEvent event, Object changedObject, ObjectState previousState,
	 ...
	 *            the previous hovered object; null if there was no hovered
	 *            object
	 ...
	 *            the previously focused object; null if there was no focused
	 *            object
