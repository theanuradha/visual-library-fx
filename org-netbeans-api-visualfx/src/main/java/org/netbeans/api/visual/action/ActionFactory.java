...
public final class ActionFactory {
	...
	// private static final ActionMapAction ACTION_MAP_ACTION = new
	// ActionMapAction (null, null);

	private static final WidgetAction MOVE_CONTROL_POINT_ACTION_FREE = createMoveControlPointAction(
			createFreeMoveControlPointProvider());

	private static final WidgetAction MOVE_CONTROL_POINT_ACTION_ORTHOGONAL = createMoveControlPointAction(
	...
	private static final WidgetAction CYCLE_FOCUS_OBJECT_SCENE = createCycleFocusAction(
			new CycleObjectSceneFocusProvider());
	...
	// * The key-to-action binding is obtained from the InputMap of a view
	...
	// /**
	// * Creates an action which handles keys and popup menu.
	// * The key-to-action binding and popup menu items are obtained from
	// specified ActionMap and InputMap.
	// * @param inputMap the input map
	// * @param actionMap the action map
	// * @return the action-map action
	// */
	// public static WidgetAction createActionMapAction (InputMap inputMap,
	// ActionMap actionMap) {
	 ...
	 */
	public static WidgetAction createAlignWithMoveAction(LayerWidget collectionLayer, LayerWidget interractionLayer,
	...
	public static WidgetAction createAlignWithMoveAction(LayerWidget collectionLayer, LayerWidget interractionLayer,
		...
		return createAlignWithMoveAction(new SingleLayerAlignWithWidgetCollector(collectionLayer, outerBounds),
				interractionLayer, decorator != null ? decorator : ALIGN_WITH_MOVE_DECORATOR_DEFAULT, outerBounds);
	}
	 ...
	 * @return the align-with move action
	 */
	public static WidgetAction createAlignWithMoveAction(AlignWithWidgetCollector collector,
	 ...
	 */
	public static WidgetAction createAlignWithResizeAction(LayerWidget collectionLayer, LayerWidget interractionLayer,
			AlignWithMoveDecorator decorator) {
	...
	public static WidgetAction createAlignWithResizeAction(LayerWidget collectionLayer, LayerWidget interractionLayer,
			AlignWithMoveDecorator decorator, boolean outerBounds) {
		assert collectionLayer != null;
		return createAlignWithResizeAction(new SingleLayerAlignWithWidgetCollector(collectionLayer, outerBounds),
				interractionLayer, decorator != null ? decorator : ALIGN_WITH_MOVE_DECORATOR_DEFAULT, outerBounds);
	}
	 ...
	 * @return the align-with resize action
	 */
	public static WidgetAction createAlignWithResizeAction(AlignWithWidgetCollector collector,
			LayerWidget interractionLayer, AlignWithMoveDecorator decorator) {
	...
	public static WidgetAction createAlignWithResizeAction(AlignWithWidgetCollector collector,
			LayerWidget interractionLayer, AlignWithMoveDecorator decorator, boolean outerBounds) {
		assert collector != null && interractionLayer != null && decorator != null;
		AlignWithResizeStrategyProvider sp = new AlignWithResizeStrategyProvider(collector, interractionLayer,
	 ...
	 * @return the connect action
	 */
	public static WidgetAction createConnectAction(ConnectDecorator decorator, LayerWidget interractionLayer,
			ConnectProvider provider) {
		assert interractionLayer != null && provider != null;
		return new ConnectAction(decorator != null ? decorator : createDefaultConnectDecorator(), interractionLayer,
				provider);
	}
	 ...
	 * @return the extended connect action
	 */
	public static WidgetAction createExtendedConnectAction(ConnectDecorator decorator, LayerWidget interractionLayer,
			ConnectProvider provider) {
		assert interractionLayer != null && provider != null;
		return new ExtendedConnectAction(decorator != null ? decorator : createDefaultConnectDecorator(),
				interractionLayer, provider, MouseEvent.CTRL_MASK);
	}
	 ...
	 */
	public static WidgetAction createExtendedConnectAction(ConnectDecorator decorator, LayerWidget interractionLayer,
			ConnectProvider provider, int modifiers) {
		assert interractionLayer != null && provider != null && modifiers != 0;
		return new ExtendedConnectAction(decorator != null ? decorator : createDefaultConnectDecorator(),
				interractionLayer, provider, modifiers);
	}
	...
	public static WidgetAction createMoveAction(MoveStrategy strategy, MoveProvider provider) {
		return new MoveAction(strategy != null ? strategy : createFreeMoveStrategy(),
				provider != null ? provider : createDefaultMoveProvider());
	}
	...
	public static WidgetAction createRectangularSelectAction(ObjectScene scene, LayerWidget interractionLayer) {
		assert scene != null;
		return createRectangularSelectAction(ActionFactory.createDefaultRectangularSelectDecorator(scene),
				interractionLayer, ActionFactory.createObjectSceneRectangularSelectProvider(scene));
	}
	 ...
	 * @return the rectangular select action
	 */
	public static WidgetAction createRectangularSelectAction(RectangularSelectDecorator decorator,
			LayerWidget interractionLayer, RectangularSelectProvider provider) {
	 ...
	 * @return the resize action
	 */
	public static WidgetAction createResizeAction(ResizeStrategy strategy, ResizeControlPointResolver resolver,
			ResizeProvider provider) {
		return new ResizeAction(strategy != null ? strategy : createFreeResizeStategy(),
				resolver != null ? resolver : createDefaultResizeControlPointResolver(),
				provider != null ? provider : createDefaultResizeProvider());
	}
	 ...
	 *            whether or not to first select the underlying widget with a
	 *            right-click.
	...
	/**
	 * Creates a contiguous select action. Trigger by any left, middle or right
	 * mouse-button. If no key-modifier is held, then it behaves as a
	 * non-contiguous replace selection. If Shift key-modifier is held, then it
	 * behaves as a contiguous replace selection. If Ctrl key-modifier is held,
	 * then it behaves as a non-contiguous additive selection. If Ctrl and Shift
	 * key-modifiers are held, then it behaves as a contiguous additive
	 * selection.
	 * 
	 * @param provider
	 *            the contiguous select logic provider
